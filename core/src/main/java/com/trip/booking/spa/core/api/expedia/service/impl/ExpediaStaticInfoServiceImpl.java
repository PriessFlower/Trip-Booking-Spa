...
import com.trip.booking.spa.core.placeholder.hotelinfo.request.QueryHotelRequest;
import com.trip.booking.spa.core.placeholder.hotelinfo.request.SupplierHotelBaseRequest;
import com.trip.booking.spa.core.placeholder.hotelinfo.response.PageResp;
...
import com.trip.booking.spa.core.api.expedia.access.HotelDetailsAccess;
import com.trip.booking.spa.core.api.expedia.access.HotelFileAccess;
import com.trip.booking.spa.core.api.expedia.access.HotelRemoveAccess;
import com.trip.booking.spa.core.api.expedia.access.QueryProductAccess;
import com.trip.booking.spa.core.api.expedia.access.RegionsAccess;
import com.trip.booking.spa.core.api.expedia.adaptor.ExpediaStaticInfoAdaptor;
import com.trip.booking.spa.core.api.expedia.bean.request.HotelInfoRequest;
...
import com.trip.booking.spa.core.api.expedia.bean.response.HotelFileResponse;
import com.trip.booking.spa.core.api.expedia.bean.response.HotelIdsResponse;
...
import com.trip.booking.spa.core.util.FileDealUtils;
    ...
    private void saveCityList(List<CityInfoRequest> cityInfoList) {
        ...
        for (int i = 0; i < cityInfoList.size(); i += batchSize) {
            ...
            try {
                ...
                hotelBasePlaceholderClient.saveCityList(saveCityInfoList);
            } catch (Exception e) {
...
//        log.info("国家：{}下地区：{}-》》》》》》》》下城市推送完毕", cityInfoList.get(0).getCountryId(), cityInfoList.get(0).getStateNameCN());
    ...
    private void saveCountryList(List<CountryInfoRequest> countryInfoList) {
        try {
            hotelBasePlaceholderClient.saveCountryList(countryInfoList);
        } catch (Exception e) {
...
//        hotelBasePlaceholderClient.saveHotelDetails(hotelDetailsRequests);
//        hotelInfoPlaceholderClient.saveHotelInfo(supplierHotelBaseRequests);
//        hotelInfoPlaceholderClient.saveRoomInfo(supplierHotelBaseRequests.stream().flatMap(supplierHotelBaseRequest -> supplierHotelBaseRequest.getRoomList().stream()).collect(Collectors.toList()));
    }
...
//    }
    private void pushBaseHotelDetails(HotelDetailsRequest hotelDetailsRequest) {
        ...
        //保存酒店详情
        long startTime = System.currentTimeMillis();
        hotelBasePlaceholderClient.saveHotelDetails(Arrays.asList(hotelDetailsRequest));
        long endTime = System.currentTimeMillis();
        log.info("推送base酒店详情耗时：{}", endTime - startTime);
    ...
    private void pushInfoHotelDetails(HotelStaticInfo hotelStaticInfoUS, HotelStaticInfo hotelStaticInfoCN) {
        ...
        //保存酒店详情
        long startTime = System.currentTimeMillis();
        hotelInfoPlaceholderClient.saveHotelInfo(Arrays.asList(supplierHotelBaseRequest));
        //保存房型信息
        hotelInfoPlaceholderClient.saveRoomInfo(supplierHotelBaseRequest.getRoomList());
        long endTime = System.currentTimeMillis();
    ...
    public void deleteHotelInfo(String deleteDate) {
        ...
        for (int i = 0; i < hotelIds.size(); i += batchSize) {
            ...
            hotelBasePlaceholderClient.removeHotelDetails(requestHotelIds);
            // 每处理完一组，增加当前批次计数器
            currentBatch++;
        }
    }
    ...
    public void saveOrUpdateProductInfo(String checkInDate, String checkOutDate, List<String> supplierHotelIds, Integer startNum) {
        ...
        while (true) {
            ...
            InfoResult<PageResp<SupplierHotelBaseResponse>> hotelInfoPageListResult = hotelInfoPlaceholderClient.queryHotelPageList(queryHotelRequest);
            ...
            ThreadPoolUtils.execute(() -> {
                ...
                try {
                    ...
                    if (null != resultOnly.getData() && CollectionUtils.isNotEmpty(resultOnly.getData().getHotelPrices())) {
                        //推送base
                        hotelBasePlaceholderClient.aggregatorProductMapping(ExpediaStaticInfoAdaptor.transformBaseProductReq(resultOnly.getData()));
                        //推送info
                        hotelInfoPlaceholderClient.saveProductInfo(ExpediaStaticInfoAdaptor.transformInfoProductReq(resultOnly.getData()));
                    } else {
                    ...
                    if (null != resultPackage.getData() && CollectionUtils.isNotEmpty(resultPackage.getData().getHotelPrices())) {
                        //推送base
                        hotelBasePlaceholderClient.aggregatorProductMapping(ExpediaStaticInfoAdaptor.transformBaseProductReq(resultPackage.getData()));
                        //推送info
                        hotelInfoPlaceholderClient.saveProductInfo(ExpediaStaticInfoAdaptor.transformInfoProductReq(resultPackage.getData()));
                    } else {
