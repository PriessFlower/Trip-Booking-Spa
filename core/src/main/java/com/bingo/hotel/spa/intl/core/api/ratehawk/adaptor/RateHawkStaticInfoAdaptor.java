package com.bingo.hotel.spa.intl.core.api.ratehawk.adaptor;
        ...
        if (CollectionUtils.isNotEmpty(roomGroups)) {
            roomGroups.forEach(room -> {
                SupplierRoomBaseRequest roomBaseRequest = new SupplierRoomBaseRequest()
                        .setSupplierId(10007)
                        .setSupplierHotelId(hotelId)
                        .setSupplierRoomId(hotelId + "_" + room.getName() + "_" + room.getRg_ext().getBedding() + "_" + room.getRg_ext().getCapacity())
            ...
            if (CollectionUtils.isNotEmpty(hotelPrice.getRates())) {
                hotelPrice.getRates().forEach(rate -> {
                    ...
                    SupplierProductBaseRequest request = new SupplierProductBaseRequest()
                            ...
                            .setSupplierProductId(hotelPrice.getHid() + "_" + rate.getRoom_name() + "_" + rate.getMeal() + "_" + (StringUtils.isBlank(cancellationPenalties.getFree_cancellation_before()) ? true : false))
