...
| P2 | 占用统一为结构化 per-room；删除未使用却标 `@NonNull` 的死字段 | 后半已做（2026-08-26 删 9 个零读写死字段：`salesType`/`guestType`(@NonNull)/`planSession`×2/`sCityCode`×2/`plansId`/`bedTypeCode`/`expediaRoomId`；`maxOccupancy` 经复核已有真实写方不删，`bedCheckInfos` 是查价内部载体另行处置）；per-room 占用待做 |
