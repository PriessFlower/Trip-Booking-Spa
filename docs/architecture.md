...
分组指纹（比价用），`subSaleId` 是售卖单元 id。后果是仓内 4 处独立拆解，其中一处用
`lastIndexOf('#')` 而另三处用 `indexOf`/`contains`，规则不一致。
...
| **身份** | 稳定、可重复解析、上游长期持有 | ❌ **仍是易腐的 `rate.id`** |

**原则：身份与令牌永不同字段。** 合在一个字段里，就会长出 `md5#subSaleId`。

### 6.3 已经做了的：验价分态

Expedia 每次查价都会重铸 `rate.id`。上游缓存列表、旅客几分钟后点进来，那份报价可能
已经不在。原实现对四种情形一律返回 `null`，现已分开：
...
- **稳定 `productKey`**：由 (property_id, room_id, 餐食, 退改, 支付方式, 床型, 占用)
  派生，`rate.id` 收进网关内部。上游的订单与快照只绑 `productKey`。
- **陈码重解析**：验价时若 `rate.id` 已不在，用 `productKey` 在当前响应里找等价报价
  重新验价，对上游无感。**硬门不可放宽——同餐、同退改必须一致**：cursor 实证过"只按
  最便宜救会把含早错配成不含早"（订单 49046202）。
...
| 产品身份仍是易腐 `rate.id`（§6.4） | 旧列表点击会得到 `RATE_DEAD` 而非自动救回 |
