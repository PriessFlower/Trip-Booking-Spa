...
- **O-7.1 (MUST)** **快照表必须先存在，才允许按 §6.1.1.1 对成功路径的大对象采样**。二者是同一笔交易的两面：不建表就采样，等于取证与现场两头落空。现状没有任何「每次调用一行」的表，而 §6.1.1.1 点名的巨型日志仍在全量直打（占生产一小时日志量 17%）。注意别被名字骗了：`expedia_property_content` 虽以快照命名，实为覆盖写的内容档案（见 O-7.2）；2026-09-08 静态内容域退役后本仓已无写入方，只剩读侧。
...
- **O-8.2 (MUST)** 诊断端点只读。会改状态的属运维动作，分置 `ops/`——现状 `ops/BackDoorController` 的 2 个端点（Expedia 建档、价格缓存查看）都是运维动作，分置本身是对的；缺的是诊断侧，一个都没有。
...
| 14 | 报文快照表不存在 | O-7.1、O-7.2 | 全仓没有「每次调用一行」的表。唯一以快照命名的 `expedia_property_content` 是内容档案不是报文证据——主键 `property_id+language`、覆盖写、无状态码无耗时，且 2026-09-08 起已无写入方 | 未修 |
| 15 | 诊断端点不存在 | O-8.1 | `ops/BackDoorController` 的 2 个端点（Expedia 建档、价格缓存查看）都是运维动作，没有一个回答“这一家为什么没出报价” | 未修 |
