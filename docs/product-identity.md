...
- **R-2.10 (MUST)** **等价类成分随报价透出，上游照用不得重判。** 出参带三样：餐食规范形
  （`mealSignature`，如 `B1L0D0`）、退改粗分类（`cancelClass`）、占用（`occupancy`）——
  与 productKey 的同名成分同源同值，由派生器一次算出（R-2.8 的同一条纪律再往外延一跳）。

  为什么必须由网关给：这三样的判定依据是各家报文，写法千差万别，归一化只在适配层做得对。
  上游拿原始描述重判一遍必然与派生器分叉，而分叉不报错——R-2.8 记的建档分叉即同款。

  **只透这三样**：账号、供应商酒店 id、供应商房型 id 不出网关。它们凑齐就能反推 productKey，
  等于把身份发号权交出去（R-1.5 禁止发号）。

  **两条路各有各的来源**：实时查价从 `identity` 直接拓（现查的产品可能还没建档）；走缓存的
  从**档案表**取——成分是稳定信息，塞进 Redis 就成了同一事实的第二份拷贝，判据一改两份必然
  对不上（R-2.6 的白名单即为此设，缓存里留 productKey 就是通往档案表的桥）。

  推论：走缓存的报价，成分随**建档覆盖率**走。未建档的产品三个字段为空，与房型/餐食属性同命运
  （看板「档案属性命中率」即这条线）。下游必须容忍空值——空是"我们还没建档"，不是"没有餐食"。

- **R-2.11 (MUST NOT)** **本仓不产出跨供应商的等价键。** productKey 含供应商码与供应商侧
  酒店/房型 id，天然不跨家；「这两家的这两个房型是同一间」属聚合域（R-2.4：映射表不建在网关）。

  跨家归并 = 聚合给的房型对照 + R-2.10 的三个成分，由比价方自己拼。网关给事实，不给结论——
  若网关代为归并，等于把「哪两家可比」这个判断从比价域偷渡进网关，而网关看不到全局供给。
