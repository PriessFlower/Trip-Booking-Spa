...
public enum SupplierIdentityProfile {
     ...
     *
     * <p>TTL 上限取 30 分钟：句柄里存的是验价 PreBook 拿到的 {@code ReferenceNo}。
     * 官方只在错误码 3006 的文案里提过一句「有效时长为2小时」，接口说明页未写；且那说的是
     * 号本身的寿命，不是房价被锁住。<b>本仓按 1 小时计</b>——这是我方选定的保守口径，不是
     * 道旅的承诺，也未向其求证；当天入住的单子还会更早死于酒店的当日截止。帽取其一半（R-2.2）。
     *
