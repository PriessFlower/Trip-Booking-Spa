...
public class ExpediaBookingSyncServiceImpl
    ...
    private BookingResult bookInternal(BookingCommand command) {
        ...
        if (StringUtils.isBlank(bookHref)) {
            log.error("expedia booking 报价句柄缺少下单链接 orderId={}, offerId={}",
                    command.orderId(), command.offerId());
            return BookingResult.failed(command.orderId(), "offer_credential_missing", "报价句柄内容不完整，请重新验价后下单", "报价句柄内容不完整，请重新验价后下单");
        }
        try {
            bookingContact.requireUsable();
        } catch (IllegalStateException e) {
            return BookingResult.failed(command.orderId(), "booking_contact_unusable", e.getMessage(), e.getMessage());
        }

        ExpediaBookingContact.Contact contact = bookingContact.getContact();
        String body = JsonUtils.writeObject2Json(buildRequest(command, contact));
        ...
        log.info("expedia booking 分类结果 orderId={}, httpStatus={}, classification={}",
                command.orderId(), httpStatus, classification);

        switch (classification) {
            case SUCCESS:
                return succeeded(command, contact, data.getItinerary_id());
            case DUPLICATE:
                // A 方案：网关内部消化，上游无感。反查取回首次已成立的订单号
                return resolveDuplicate(command, contact);
            case DETERMINISTIC_FAILURE:
                return BookingResult.failed(command.orderId(), data == null ? String.valueOf(httpStatus) : data.getType(), data == null ? "供应商拒单" : data.getMessage(), data == null ? "供应商拒单" : data.getMessage());
            case INDETERMINATE:
            default:
                return BookingResult.unknown(command.orderId(), data == null ? String.valueOf(httpStatus) : data.getType(), "下单结果不确定，请稍后凭订单号反查确证", "下单结果不确定，请稍后凭订单号反查确证");
        }
    }

    /** 下单成功后补齐确认号；反查失败不改变已确定的成功结论 */
    private BookingResult succeeded(BookingCommand command, ExpediaBookingContact.Contact contact, String itineraryId) {
        // 先反查再构造：BookingResult 不可变，没有"造完再补一个字段"这条路。
        // 反查失败时确认号留空，不改变已确定的成功结论
        QueryOrderResponse.Itinerary itinerary = queryQuietly(command, contact);
        return BookingResult.success(command.orderId(), itineraryId,
                itinerary == null ? null : firstConfirmationNumber(itinerary), null);
    }
     ...
     */
    private BookingResult resolveDuplicate(BookingCommand command, ExpediaBookingContact.Contact contact) {
        QueryOrderResponse.Itinerary itinerary = queryQuietly(command, contact);
        if (itinerary != null && StringUtils.isNotBlank(itinerary.getItinerary_id())) {
            log.info("expedia booking 重复下单已收敛为成功 orderId={}, itineraryId={}",
                    command.orderId(), itinerary.getItinerary_id());
            // 确认号与单号一同构造：BookingResult 不可变，没有"造完再补一个字段"这条路
            return BookingResult.success(command.orderId(), itinerary.getItinerary_id(),
                    firstConfirmationNumber(itinerary), null);
        }
        log.warn("expedia booking 报重复下单但反查未取回订单，退为不确定 orderId={}", command.orderId());
        return BookingResult.unknown(command.orderId(), ExpediaBookingClassifier.DUPLICATE_ITINERARY, "供应商报订单已存在，但反查未能取回，请稍后重试反查", "供应商报订单已存在，但反查未能取回，请稍后重试反查");
    }

    /** 反查一次，任何异常都只记录不外抛——调用方的结论不应因补充信息失败而改变 */
    private QueryOrderResponse.Itinerary queryQuietly(BookingCommand command, ExpediaBookingContact.Contact contact) {
        try {
            ResponseResult<QueryOrderResponse> qr = new QueryOrderAccess(
                    host, command.orderId(), contact.getEmail(), expediaUtils.generateSign(),
            ...
            if (qd == null || qd.getPresence() != QueryOrderResponse.Presence.FOUND) {
                log.info("expedia booking 反查未取回订单 orderId={}, presence={}",
                        command.orderId(), qd == null ? null : qd.getPresence());
                return null;
            }
            return qd.firstItinerary();
        } catch (Exception e) {
            log.error("expedia booking 反查异常 orderId={}", command.orderId(), e);
            return null;
        }
    }
    ...
    private CreateOrderRequest buildRequest(BookingCommand command, ExpediaBookingContact.Contact contact) {
        ...
        for (int i = 0; i < command.roomNum(); i++) {
        ...
        return CreateOrderRequest.builder()
                .affiliate_reference_id(command.orderId())
                .hold(false)
