 ...
 *
 * <p><b>每组留 2 条而非 1 条</b>：为 resolve 换票留备份票——一个 productKey 对应
 * 0..N 张在售票本就是常态（R-1.4），只留 1 条会让首选票一死就无票可换。
 *
 * <p><b>与异常价拦截的语义差别</b>：被 {@link AbnormalPriceGuard} 拦下的产品要
 * <b>排除</b>出下架逻辑（我们不确定新价对错，故保留旧价）；而被本类裁掉的产品
 * <b>应当</b>走下架逻辑置 0——那是我方主动选择不再提供该报价，客人拿旧 productId
...
public class PriceCacheTrimmer {
     ...
     */
    @Value("${cache.price.keep-per-product-key:2}")
