...
public class DidaOrderQuerySyncServiceImpl extends AbstractOrderQuerySyncSupportService<DidaBookingSearchResponse> {
    ...
    @Override
    public DidaBookingSearchResponse doOrderQuery(OrderQueryCommand req) {
        if (!properties.isConfigured()) {
            log.error("道旅查单：凭证未配置,orderId={}", req.orderId());
            return null; // 模板兜底为「结果不确定」
        }
        String bookingId = StringUtils.trimToNull(req.supplierOrderId());
        ResponseResult<DidaBookingSearchResponse> result = new BookingSearchAccess(properties).access(
                bookingId != null ? BookingSearchAccess.byBookingId(properties, bookingId)
                        : BookingSearchAccess.byClientReference(properties, req.orderId()),
                CallPurpose.ORDER);
        return result == null ? null : result.getData();
    }

    @Override
    public OrderQueryResult orderQueryRespConvert(DidaBookingSearchResponse resp) {
        if (!resp.isSucc()) {
            ...
            return OrderQueryResult.indeterminate("查单未取得确定结果(" + code + "：" + resp.errorMessage() + ")");
        }
        ...
        if (hits != 1) {
            ...
            return OrderQueryResult.indeterminate(
                    hits == 0 ? "供应商未返回该订单（空返回不能视为不存在，可能仍在建单），请稍后重试查单"
                            : "供应商返回了 " + hits + " 笔订单，坐标有歧义，请携带供应商单号重查");
        }
        DidaBookingDetails d = resp.bookings().get(0);
        OrderState state = DidaBookingClassifier.toOrderState(d.getStatus());
        if (state == null) {
