 ...
 * 反查确证。故入参只需 {@link OrderQueryCommand#orderId()}，不要求供应商订单号——那种场景下
 * 上游本来就没有它。
 *
...
public class ExpediaOrderQuerySyncServiceImpl
    ...
    @Override
    public QueryOrderResponse doOrderQuery(OrderQueryCommand command) {
        // 邮箱必须与下单时一致，否则 Expedia 不返回结果；故取同一份配置，不由调用方传入
        String email = bookingContact.getContact().getEmail();
        if (email == null || email.isBlank()) {
            // 配置缺失时不能报「订单不存在」——那会诱导上游重新下单
            log.error("expedia 查单缺少 booking-contact 邮箱，无法反查, orderId={}", command.orderId());
            return null;
        }
    ...
    @Override
    public OrderQueryResult orderQueryRespConvert(QueryOrderResponse resp) {
        if (resp.getPresence() != QueryOrderResponse.Presence.FOUND) {
            return resp.getPresence() == QueryOrderResponse.Presence.NOT_FOUND
                    ? OrderQueryResult.notFound(resp.getMessage())
                    : OrderQueryResult.indeterminate(resp.getMessage());
        }
        ...
        if (itinerary == null || itinerary.getItinerary_id() == null || itinerary.getItinerary_id().isBlank()) {
            ...
            return OrderQueryResult.indeterminate("查单响应自相矛盾：报告订单存在但未给出订单号");
        }
     ...
     */
    static OrderState mapOrderStatus(List<CreateOrderResponse.Room> rooms) {
        ...
        return anyPending ? OrderState.BOOKING : null;
    }
