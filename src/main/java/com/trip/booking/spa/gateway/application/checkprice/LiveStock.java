...
public final class LiveStock<S> {
    ...
    private Function<PriceQuery, List<Product>> freshConverter;
     ...
     * 由适配层在此处捕获原始响应，模板只需要一个「PriceQuery → 报价」的函数。
     *
     ...
     */
    public LiveStock<S> freshConvertedBy(Function<PriceQuery, List<Product>> converter) {
        this.freshConverter = converter;
        return this;
    }

    public Function<PriceQuery, List<Product>> freshConverter() {
